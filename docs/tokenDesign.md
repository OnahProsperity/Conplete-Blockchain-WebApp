...
The DYCOIN Fiat Token contract is an ERC-20 compatible token. It allows
...
DYCOIN will control the address of all roles except for minters, which will be
controlled by the entities that DYCOIN elects to make minters
...
entities will have to be members of DYCOIN, and will be vetted by DYCOIN before
they are allowed to create new tokens. DYCOIN will not mint any tokens itself,
it will approve members to mint and burn tokens.

Each `minter` has a `mintingAllowance`, which DYCOIN configures. The
`mintingAllowance` is how many tokens that minter may issue, and as a `minter`
issues tokens, its `mintingAllowance` declines. DYCOIN will periodically reset
the `mintingAllowance` as long as a `minter` remains in good standing with
DYCOIN and maintains adequate reserves for the tokens it has issued. The
`mintingAllowance` is to limit the damage if any particular `minter` is
compromised.

### Adding Minters

DYCOIN adds minters via the `configureMinter` method. When a minter is
...
continue minting. When a `minter`'s allowance is low, DYCOIN can make another
call to `configureMinter` to reset the `mintingAllowance` to a higher value.

### Removing Minters

DYCOIN removes minters via the `removeMinter` method. This will remove the
...
DYCOIN blacklists an address via the `blacklist` method. The specified `account`
...
DYCOIN removes an address from the blacklist via the `unblacklist` method. The
...
that caused DYCOIN to pause the contract.

### Pause

DYCOIN will pause the contract via the `pause` method. This method will set the
paused flag to true.
...
DYCOIN will unpause the contract via the `unpause` method. This method will set
